                                               ...
                                               res.status(200).json({
                                                         success: true,
                                                          data: 'created'
                                                               ...
                                                               // let filename=req.files.file.name+path.parse(req.files.file.name).ext;
     req.files.file.mv(`${process.env.file_upload_path}/${filename}`,function(err){
     if(err){
...
const {customer_id,customer_name,customer_email,customer_pass,customer_country,customer_city,customer_contact,customer_address}= req.body
        // let salt= await bcrypt.genSalt(10);
    //  customer_pass=await bcrypt.hash(customer_pass,salt)
                     if(!customer_id){
                      next(new ErrorResponse('Please enter customer_id',404));
                     }
                           connectDB().request().query(`insert into customers (customer_id,customer_name,customer_email,customer_pass,customer_country,customer_city,
                                customer_contact,customer_address) values ('${customer_id}','${customer_name}','${customer_email}','${customer_pass}','${customer_country}','${customer_city}','${customer_contact}','${customer_address}')`, function (err) {
                              if (err) {
                                next(err);
                              }
                              else {
