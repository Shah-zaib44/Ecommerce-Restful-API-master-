...
function showProducts(){
    (async () => {
        ...
        console.log(article.data.recordset.length)
        let i=0;
  while(i<article.data.recordset.length){
                      var a=`<div class="shop-item col-4">
        <span class="shop-item-title">${article.data.recordset[i].product_title}</span>
        <br>
        <img class="shop-item-image" src="Images/bunk.png">
        <div class="shop-item-details">
            <span class="shop-item-price">${article.data.recordset[i].product_id}</span>
            <br>
            <button class="btn btn-primary shop-item-button" type="button">ADD TO CART</button>
            <br>
        </div>
    </div>`;
    var b=document.querySelector('.shop-items');
    b.innerHTML+=a;        
i++;
       }
    })();
